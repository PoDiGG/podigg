...
class ParameterizedConnectionsGenerator extends ConnectionsGenerator {
    constructor(region, routes, pointToLatLon, config) {
        ...
        if (this.param_time_final <= this.param_time_initial) {
            throw new Error('The final time must be higher than the initial time.');
        }
    ...
    generate() {
        ...
        while (this.connections.length < this.param_connections) {
            ...
            if (initial > this.param_time_final) {
                if (!this.connections.length && tries++ % 100000 === (100000 - 1)) {
                    console.error('Skipped more than 1000000 connections because of a narrowly defined time range, ' +
                      'increasing time_final or decreasing time_initial is advised.');
                }
                continue;
            }
            ...
            var stopSequence = 0;
            toAddConnections.forEach(connection => this.addConnection(connection.trip, connection.departureTime,
              connection.arrivalTime, connection.delayDeparture, connection.delayArrival,
              connection.delayDepartureReason, connection.delayArrivalReason, connectionSequenceId, stopSequence++));
