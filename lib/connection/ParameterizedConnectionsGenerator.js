...
class ParameterizedConnectionsGenerator extends ConnectionsGenerator {
    constructor(region, routes, config) {
        ...
        this.param_time_initial = Config.value(config, 'time_initial', 0); // The initial timestamp
        this.param_time_final = Config.value(config, 'time_final', 24 * 3600000); // The final timestamp
        this.param_connections = Config.value(config, 'connections', 30000); // The number of connections to generate

        this.param_route_choice_power = Config.value(config, 'route_choice_power', 2); // Higher values means higher chance on larger routes when selecting routes
        this.param_time_choice_power = Config.value(config, 'time_choice_power', 3); // Higher values means higher chance on more frequent times when selecting times
        this.param_vehicle_max_speed = Config.value(config, 'vehicle_max_speed', 160); // km/h
        this.param_vehicle_speedup = Config.value(config, 'vehicle_speedup', 5000); // km/(h^2)
        ...
        this.param_hourly_weekday_distribution = Config.value(config, 'hourly_weekday_distribution', [
            0.05,
            0.01,
            0.01,
            0.48,
        ...
        this.param_hourly_weekend_distribution = Config.value(config, 'hourly_weekend_distribution', [
            0.09,
            0.01,
            0.01,
            0.08,
    ...
    getTripDuration(trip) {
        var km_per_degree = 1.3218125741524802;
        return this.getDistanceDuration(DistanceHelpers.point2D(trip.from, trip.to) * km_per_degree) * 3600000;
    }
