...
class QuerySetGenerator {
    constructor(region, config) {
        ...
        this.param_time_choice_power = Config.value(config, param_scope + 'time_choice_power', 1); // Higher values means higher chance on more frequent times when selecting times
        ...
        this.sorted_week_hours = Time.sortHours(this.param_hourly_weekday_distribution);
        this.sorted_weekend_hours = Time.sortHours(this.param_hourly_weekend_distribution);
    }
    ...
    getRandomInitialTime() {
        return Time.getRandomInitialTime(this.getRandomClosure(), this.sorted_week_hours, this.sorted_weekend_hours, this.param_time_initial, this.param_time_final, this.param_time_choice_power);
    }
