...
class ParameterizedEdgesGenerator extends EdgesGenerator {
    constructor(region, config) {
        ...
        this.param_max_intracluster_distance = Config.value(config, 'max_intracluster_distance', 100); // The maximum distance stations in one cluster can have from each other.
        this.param_max_intracluster_distance_growthfactor = Config.value(config, 'max_intracluster_distance_growthfactor', 0.1); // The factor the param_max_intracluster_distance should grow in each step. (until 1 is reached) The lower this value, the larger the chance that closer stations will be clustered first before further away stations.
        this.param_post_cluster_max_intracluster_distancefactor = Config.value(config, 'poster_cluster_max_intracluster_distancefactor', 1.5); // Maximum distance clusters can have in the post step. The larger the value, the larger the chance that a cluster will be connected to more clusters.

        this.param_loosestations_neighbourcount = Config.value(config, 'loosestations_neighbourcount', 3); // The number of neighbours around a loose station that should define its area.
        this.param_loosestation_max_range_factor = Config.value(config, 'loosestations_max_range_factor', 0.3); // The maximum range to check around a loose station relative to the total region size.
        this.param_loosestation_max_iterations = Config.value(config, 'loosestations_max_iterations', 10); // The max number of iterations for one loose station.
        this.param_loosestation_search_radius_factor = Config.value(config, 'loosestations_search_radius_factor', 0.5); // The number to multiply with the area size to get the search radius for each step.
