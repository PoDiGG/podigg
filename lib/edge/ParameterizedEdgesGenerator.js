...
class ParameterizedEdgesGenerator extends EdgesGenerator {
    constructor(region) {
        ...
        this.param_post_cluster_max_intracluster_distancefactor = 1.5; // Maximum distance clusters can have in the post step. The larger the value, the larger the chance that a cluster will be connected to more clusters.

        this.param_loosestations_neighbourcount = 3; // The number of neighbours around a loose station that should define its area.
        this.param_loosestation_max_range_factor = 0.3; // The maximum range to check around a loose station relative to the total region size.
        this.param_loosestation_max_iterations = 10; // The max number of iterations for one loose station.
        this.param_loosestation_search_radius_factor = 0.5; // The number to multiply with the area size to get the search radius for each step.
                    ...
                    while (count++ < this.param_loosestation_max_iterations && DistanceHelpers.point2D(search_center, station) < avgDistance) {
                        ...
                        if (points.length > 0) {
                            ...
                            if (DistanceHelpers.point2D(station, point) > 0) {
                                this.edges.push({from: station, to: point});
                            }
                        }
                    }
                }
