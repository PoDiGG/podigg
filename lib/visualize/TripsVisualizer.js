    ...
    render(filename) {
        ...
        for (var x = 0; x < size.x; x++) {
            for (var y = 0; y < size.y; y++) {
                var value_center = this.region.getRaw(x, y);
                var value_up = this.region.getRaw(x, y + 1);
                var value_down = this.region.getRaw(x, y - 1);
                var value_left = this.region.getRaw(x - 1, y);
                var value_right = this.region.getRaw(x + 1, y);
                if ((value_center && value_center.station)
                  || (value_up && value_up.station)
                  || (value_down && value_down.station)
                  || (value_left && value_left.station)
                  || (value_right && value_right.station)) {
                    // MDC ImageData:
