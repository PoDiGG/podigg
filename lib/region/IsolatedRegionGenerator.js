...
class IsolatedRegionGenerator extends RegionGenerator {
    constructor(config) {
        super(config);
        this.param_pop_clusters = Config.value(config, 'pop_clusters', 50);
        this.param_pop_average = Config.value(config, 'pop_average', 0);
        this.param_pop_deviation = Config.value(config, 'pop_deviation', 10);
        this.param_max_radius = Config.value(config, 'max_radius', 50);
    }
