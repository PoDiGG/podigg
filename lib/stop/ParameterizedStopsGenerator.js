...
class ParameterizedStopsGenerator extends StopsGenerator {
    constructor(region, config) {
        ...
        this.param_min_station_size = Config.value(config, 'min_station_size', 0.01); // The minimum population density for a station to form
        this.param_max_station_size = Config.value(config, 'max_station_size', 30); // The maximum population density for a station to form
        this.param_start_stop_choice_power = Config.value(config, 'start_stop_choice_power', 4); // Higher values means higher chance on larger stations when selecting starting stations
        this.param_min_interstop_distance = Config.value(config, 'min_interstop_distance', 1); // The minimum distance between stops in number of cells.
        this.param_factor_stops_post_edges = Config.value(config, 'factor_stops_post_edges', 0.66);

        this.param_edge_choice_power = Config.value(config, 'edge_choice_power', 2); // Higher values means higher chance on longer edges when selecting edges in the post step
        this.param_stop_around_edge_choice_power = Config.value(config, 'stop_around_edge_choice_power', 4);// Higher values means higher chance on larger stations when selecting stations on edges
        this.param_stop_around_edge_radius = Config.value(config, 'stop_around_edge_radius', 2); // The radius to select points from, from the random point on an edge
    }
