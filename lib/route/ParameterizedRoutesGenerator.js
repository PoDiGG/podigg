...
class ParameterizedRoutesGenerator extends RoutesGenerator {
    constructor(region, edges, config) {
        ...
        this.param_routes = Config.value(config, 'routes', 1000); // The number of routes to generate
        this.param_largest_stations_fraction = Config.value(config, 'largest_stations_fraction', 0.25); // The fraction of (largest) stations inbetween which routes need to be formed.
        this.param_penalize_station_size_area = Config.value(config, 'penalize_station_size_area', 10); // The area in which station sizes should be penalized.
        this.param_max_route_length = Config.value(config, 'max_route_length', 10); // The maximum number of edges a route can have, the larger, the slower this generator.
        this.param_min_route_length = Config.value(config, 'min_route_length', 4); // The minimum number of edges a route must have.
    }
