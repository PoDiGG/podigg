...
class RandomRoutesGenerator extends RoutesGenerator {
    constructor(region, edges, config) {
        ...
        this.param_routes = Config.value(config, 'routes', 4500); // The number of routes to generate
        this.param_max_route_length = Config.value(config, 'max_route_length', 50); // The maximum route length in number of edges
    }
